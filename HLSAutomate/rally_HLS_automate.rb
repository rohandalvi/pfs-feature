...
def setup_logger(doc) 
	...
	logtext = doc.elements['/hls-rally-automate/service/log-level'].text.strip
	logfile = doc.elements['/hls-rally-automate/service/log-file'].text.strip
...
def process(doc,filename)
	...
	log.info("logging to file #{doc.elements['/hls-rally-automate/service/log-file'].text.strip.strip}")
...
ARGV.each do |arg|
	if  File::exists?( arg )
		...
		scheduler.schedule_every( 
			doc.elements['/hls-rally-automate/service/interval'].text.strip+'s') { process(doc,arg) }
	else
		p "#{arg} not found!"
	end
end
